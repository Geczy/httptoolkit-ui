...
function getStatusPill(interceptor: Interceptor) {
  if (interceptor.uiConfig?.customPill) {
    ...
    return <StatusPill color="#4caf7d">Activated</StatusPill>;
    ...
    if (interceptor.isSupported) {
      ...
      return <StatusPill color="#e1421f">Coming soon</StatusPill>;
    }
  } else {
    return null;
  }
...
export class InterceptOption extends React.Component<InterceptOptionProps> {
        ...
        {ConfigComponent && expanded ? (
          ...
          <>
            ...
            {interceptor.inProgress && (
              <LoadingOverlay>
                <Icon icon={["fac", "spinner-arc"]} size="4x" spin={true} />
              </LoadingOverlay>
            )}
          </>
        )}
  ...
  activateInterceptor = (activationOptions: unknown = {}) => {
    ...
    return interceptorStore!.activateInterceptor(
      interceptor.id,
      activationOptions
    );
  };
  ...
  @action.bound
  onClick() {
    const { onActivationStarted, activateInterceptor, onActivationSuccessful } =
      this;
